...
class Utilities
{
    ...
    public static function registrationsChat()
    {
        // Get Financial date
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $totalIncome = PaymentDetail::select(
                DB::raw("sum(payment_amount) as totalPayment"))
                ->whereMonth('created_at', '=', $startDate->month)
                ->whereYear('created_at', '=', $startDate->year)->get();
            foreach($totalIncome as $total){
                $totalAmount = $total->totalPayment;
                // dd($totalAmount);
            }
            $data[] = ['month' => $startDate->format('Y-m'), 'registrations' => $members, 'totalincom' => $totalAmount ];
            $startDate->addMonth();
        }
